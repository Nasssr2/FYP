...
<head>
  ...
  <link rel="stylesheet" href="css/buttons_need_change.css">
...
<?php include('header.php'); ?>
  ...
  <div id="searchResult">
    ...
    <form action="my_record_lost_post_change_fun.php" method="post" enctype="multipart/form-data">
    <fieldset id="PetData">
      <legend>寵物資料:</legend>
      <input type="hidden" name="Pet_ID" value="<?php echo$rec['Pet_ID']?>" readonly/>
      <input type="hidden" name="lost_ID" value="<?php echo$rec['lost_ID']?>"readonly/>
      <label>寵物名稱:</label><label><input type="text" name="Pet_Name" title="Pet_Name" autocomplete="off" value="<?php echo$rec['Pet_Name']?>"></textarea></label><br>
      <label>種類:</label><label><input type="text" name="Pet_Types" title="Pet_Types" autocomplete="off" value="<?php echo$rec['Pet_Types']?>"></textarea></label><br>
      <label>年齡:</label><label><input type="number" name="Pet_Age" autocomplete="off" title="年齡" max="50" min="0" value="<?php echo$rec['Pet_Age']?>"></label><br>
      <label>品種:</label><label><input type="text" name="Pet_Description" id="petVariety" autocomplete="off" title="品種" value="<?php echo$rec['Pet_Description']?>"></label><br>
      <label>性別:</label><label><input type="text" name="Pet_Sex" checked  value="<?php echo$rec['Pet_Sex']?>"></label><br>
      <label>體重:</label><label><input type="number" name="Pet_weight" autocomplete="off" title="體重" step="0.1" max="50" min="0"  value="<?php echo$rec['Pet_weight']?>"></label><br>
      <label>晶片:</label><label><input type="text" name="Pet_chip" checked  value="<?php echo$rec['Pet_chip']?>"></label><br>
      <label>顏色:</label><label><input type="text" name="Pet_color" id="petVariety" autocomplete="off" title="顏色"  value="<?php echo$rec['Pet_color']?>"></label><br>
      <label>特徵:</label><label><input type="text" name="Pet_feature" autocomplete="off" maxlength="50" title="特徵"  value="<?php echo$rec['Pet_feature']?>"></label><br>
    </fieldset>
    <fieldset id="Contact">
      <legend>聯絡人資料:</legend>
      <label>聯絡人:</label><label><input type="text" name="Pet_feature_fake" autocomplete="off" maxlength="50" title="特徵"  value="<?php echo$rec['name']?>"></label><br>
      <label>電話:</label><label><input type="number" name="Pet_weight_fake" autocomplete="off"  value="<?php echo$rec['phone_Number']?>"></label><br>
    </fieldset>
    <fieldset id="LostDayTime">
      <legend>走失日期:</legend>
      <label>日期:</label><label><input type="date" name="date" value="<?php echo $rec['date']; ?>" ></label><br>
      <label>時間:</label><label> <input type="time" name="time" value="<?php echo $rec['time']; ?>" ></label><br>
    </fieldset>
    <fieldset id="LostPlace">
      <legend>走失地點:</legend>
      <label>地區:</label><label><input type="text" name="area" value="<?php echo $rec['area']; ?>" ></label><br>
      <label>位置:</label><label><input type="text" name="Place" value="<?php echo $rec['Place']; ?>" ></label><br>
      <label>詳情:</label><label><input type="text" name="Details" value="<?php echo $rec['Details']; ?>" ></label><br>
    </fieldset>
    <fieldset id="LostPlaceMap">
      ...
      <button type="submit" value="Add Record" class="button1" >修改數據</button>
      <button type="reset" value="Clear Form" class="button1">重設</button>
      <button onclick="location.href='my_record.php'" type="button">返回</button>
    </form>
