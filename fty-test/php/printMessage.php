...
while ($rc = mysqli_fetch_assoc($result)) {
  extract($rc);
    $s = $s.'<a href="lost_post_view.php?Pet_ID='.$Pet_ID.'"><div class="message" style="color:black;text-align:left"><b class="sendPerson">'.$send_person_name.'&nbsp<b>在&nbsp'.$Pet_Name.'&nbsp的貼文發表了一則留言<br><span class="sendDayTime">'.$reply_dateTime.'</span></div></a>';
}

$s = $s .'<br><br><span class="messageTypeLabel">之前的通知</span><br>';
